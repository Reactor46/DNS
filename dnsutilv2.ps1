param([String] $dnsqtype  = $(throw "Please specify the DNS Query Type"),[String] $IpParam  = $(throw "Please specify the IPaddress"),[string] $RBLlist)
...
switch ($dnsqtype.ToUpper()){
	...
	RBL {$ipIpaddressSplit = $IpParam.Split(".")
	      $revipaddress = $ipIpaddressSplit.GetValue(3) + "." + $ipIpaddressSplit.GetValue(2) + "." + $ipIpaddressSplit.GetValue(1) + "." + $ipIpaddressSplit.GetValue(0) + "." + $RBLlist
	      $qrQueryresults = [PAB.DnsUtils.DNS]::GetRecords($revipaddress,$dnsqtype.ToUpper())
	...
	MULTIRBL {
	      ...
	      Foreach ($rblprov in $Rblprovders){
	     	 ...
	     	 $revipaddress = $ipIpaddressSplit.GetValue(3) + "." + $ipIpaddressSplit.GetValue(2) + "." + $ipIpaddressSplit.GetValue(1) + "." + $ipIpaddressSplit.GetValue(0) + "." + $rblprov
	     	 $qrQueryresults = [PAB.DnsUtils.DNS]::GetRecords($revipaddress,$dnsqtype.ToUpper())
		 $qresultprn = ""
	     	 foreach ($qresult in $qrQueryresults) {
	      			$qresultprn = $qresultprn  + " " + $qresult
