# Get-DNSandWINS_withDHCP.ps1
# Written by Bill Stewart (bstewart@iname.com) modified by Jason Deaver 4-25-14 (jason.deaver@live.com) 
      ...
      foreach ( $adapterAddress in $_.IPAddress ) {
        if ( $adapterAddress -match '(\d{1,3}\.){3}\d{1,3}' ) {
          foreach ( $dnsServerAddress in $_.DNSServerSearchOrder ) {
            ...
            } | select-object ComputerName,IPAddress,DNSServer,WINSPrimaryServer,WINSSecondaryServer,DHCPEnabled
          }
        }
      }
    }
